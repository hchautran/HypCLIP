...
class BaseModel(nn.Module):
    ...
    def dist_func(self, x, y, device='gpu'):
        if self.manifold_name == EUCLID:
            ...
            return eu_dis, -self.manifold.dist_batch(x, y, device=device)
        else: 
            x = F.normalize(self.manifold.logmap0(x),p=2, dim=-1) 
            y = F.normalize(self.manifold.logmap0(y),p=2, dim=-1) 
            eu_dis = torch.matmul(x, y.t()) 
            # print('calculating lorentz distance')
            return eu_dis, -self.manifold.sqdist_batch(x, y)
    ...
    def itc_loss(self, image_embeds , text_embeds):
        ...
        margin_loss = self.margin_loss(sims_i2t, sims_t2t - eye_mask)  +  self.margin_loss(sims_t2i, sims_i2i - eye_mask)
