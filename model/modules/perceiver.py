...
class FuseMultiModalLayer(nn.Module):
    def __init__(self, config:PerceiverConfig, vision_sizes, text_sizes, latent_size) -> None:
        super().__init__()
        self.vision_layers = nn.ModuleList([PerceiverLayer(
            config,
            kv_dim=vision_size,
            q_dim=latent_size,
            is_cross_attention=True,
            ...
            num_heads=config.num_cross_attention_heads,
            widening_factor=config.cross_attention_widening_factor,
        ) for vision_size in vision_sizes])

        self.text_layers = nn.ModuleList([PerceiverLayer(
            config,
            kv_dim=text_size,
            q_dim=latent_size,
            is_cross_attention=True,
    ...
    def forward(self, text_inputs:List[torch.Tensor], vision_inputs:List[torch.Tensor], text_question, vision_question , self_attend_mask=None):
        text_output = [self.text_layer[i](
            text_question,
            inputs=text_input,
        )[0] for  i ,text_input  in enumerate(text_inputs)]

        vision_output = [self.vision_layer[i](
            vision_question,
            inputs=vision_input,
        )[0] for i, vision_input in enumerate(vision_inputs)] 

        text_state = torch.cat(text_output, dim=1)
        vision_state = torch.cat(vision_output, dim=1)
...
class MultiModalModel(nn.Module):
    def __init__(self, config:PerceiverConfig, d_vision, d_text, num_blocks, mapper=None ) -> None:
        ...
        self.question = nn.Parameter(
            torch.empty(config.num_latents, config.d_latents)
        )
        nn.init.kaiming_normal_(self.question.data)
...
class FuseMultiModalModel(nn.Module):
    def __init__(self, config:PerceiverConfig, d_vision, d_text, num_blocks, mapper=None ) -> None:
        super().__init__()
        self.num_blocks = num_blocks 
        self.num_latents = config.num_latents
